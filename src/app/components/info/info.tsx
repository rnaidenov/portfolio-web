  ...
  return (
    <div className={`h-1/2 overflow-hidden md:h-unset flex justify-between items-end absolute ${shouldShow ? 'visible' : 'invisible'} uppercase w-full bottom-0 left-0 pb-4 px-8`}>
      <div className="flex gap-8">
        ...
        <ul className={`relative flex flex-col items-start md:flex-row md:items-center gap-2 ${isNavHovered ? 'text-outer-space' : 'text-flamy-orange'}`}>
          <motion.ul
            id='work'
            className={`flex-col items-start md:flex-row md:items-center ${styles.mask} absolute flex items-center gap-2 ${isNavHovered ? 'text-outer-space' : 'text-flamy-orange'}`}
            ref={maskRef}
      ...
      <div className="flex relative items-end w-6 h-24">
        ...
        <div className={`absolute bottom-full right-0 flex flex-col items-end gap-4 pb-4`}>
          <a
            id='phone'
            href="tel:+359876703632"
            target="_blank"
            onMouseEnter={handleMouseEnter}
            onMouseLeave={handleMouseLeave}
            className={`transition transition-all transition-500 ${(hovered == 'phone' || isNavHovered) ? 'text-outer-space scale-95' : 'text-flamy-orange scale-100'}`}
          >
          ...
          <a
            id='linkedin'
            href="https://www.linkedin.com/in/radoslavnaydenov/"
            target="_blank"
            onMouseEnter={handleMouseEnter}
            onMouseLeave={handleMouseLeave}
            className={`transition transition-all transition-500 ${(hovered == 'linkedin' || isNavHovered) ? 'text-outer-space scale-95' : 'text-flamy-orange scale-100'}`}
          >
